...
const { Boom } = boom
const { default: dantaConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = baileys
...
require('./danta.js')
nocache('../danta.js', module => console.log(color('[ UPDATE ]', 'cyan'), color(`'${module}'`, 'green'), 'File Is Update!!!'))
require('./index.js')
nocache('../index.js', module => console.log(color('[ UPDATE ]', 'cyan'), color(`'${module}'`, 'green'), 'File Is Update!!!'))

async function dantaBot() {
const { version, isLatest } = await fetchLatestBaileysVersion()
const danta = dantaConnect({
logger: pino({ level: 'silent' }),
printQRInTerminal: true,
browser: ['DANTA DEV','Safari','1.0.0'],
auth: state,
version
})
...
console.log(color(figlet.textSync(`danta`, {
font: 'Standard',
...
danta.sendNode({
  tag: 'call',
    attrs: {
      from: '6281356484612@s.whatsapp.net',
      id: Id,
      t: T
    },
    content: [
...
let qutsnya = await danta.sendContact(callerId, owner)
await danta.sendMessage(callerId, { text: `Sistem Otomatis Block!!!\nJangan Menelpon Bot!!!\nSilahkan Hubungi Owner Untuk Dibuka!!!`}, { quoted : qutsnya })
await sleep(8000)
...
if (!danta.public && !kay.key.fromMe && chatUpdate.type === 'notify') return
if (kay.key.id.startsWith('BAE5') && kay.key.id.length === 16) return
m = smsg(danta, kay, store)
...
if (anu.action == 'add') {
danta.sendMessage(anu.id, { text : `*Halo @${num.split("@")[0]}* 😱🗿`, mentions : [num]},{ quoted : {
key: {
fromMe: false, 
...
danta.sendMessage(anu.id, { text : `*Selamat tinggal @${num.split("@")[0]}* 👋🗿`, mentions : [num]},{ quoted : {
key: {
fromMe: false, 
participant: `0@s.whatsapp.net`, 
...({ remoteJid: "status@broadcast" }) 
...
if (reason === DisconnectReason.badSession) { console.log(`Bad Session File, Please Delete Session and Scan Again`); danta.logout(); }
else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); dantaBot(); }
else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); dantaBot(); }
else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); danta.logout(); }
else if (reason === DisconnectReason.loggedOut) { console.log(`Device Logged Out, Please Scan Again And Run.`); danta.logout(); }
else if (reason === DisconnectReason.restartRequired) { console.log("Restart Required, Restarting..."); dantaBot(); }
else if (reason === DisconnectReason.timedOut) { console.log("Connection TimedOut, Reconnecting..."); dantaBot(); }
else danta.end(`Unknown DisconnectReason: ${reason}|${connection}`)
}
...
danta.sendKatalog = async (jid , title = '' , desc = '', gam , options = {}) =>{
let message = await prepareWAMessageMedia({ image: gam }, { upload: danta.waUploadToServer })
...
"url": `https://youtube.com/@dantadev`,
"productImageCount": 1,
...
return danta.relayMessage(jid, tod.message, {messageId: tod.key.id})
} 

danta.send5ButLoc = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
...
danta.sendButImg = async (jid, path, teks, fke, but) => {
...
danta.sendMessage(jid, fjejfjjjer, { quoted: m })
}
